 ...
 */
function ScoreLine({ topic, judge, candidate, candidateId, candidateNum, prevScore, candidateGender, lockFunc }) {
                ...
                {localStorage.getItem('judge') === judge && localStorage.getItem(topic + candidateNum + candidateGender) === candidateId ? (<div className='flex gap-6' >{localStorage.getItem(`${topic + candidateNum + candidateGender}score`).split(" ").map((num, i) => (
                    ...
                    {topic === 'swimwear' ? (
                        ...
                        <form onSubmit={handleSubmit(submitScore)} className='flex w-fit h-14 gap-12'>
                            <input className={`border-2 relative w-64 p-3 text-md font-medium rounded-3xl ${errors.questionintelligence && "border-l-[2rem] border-red-400"} transition-all`} type="number" placeholder='Intelligence' {...register("questionintelligence", { required: true, max: 50, min: 0, maxLength: 2 })} />

                            <input className={`border-2 w-64 relative p-3 text-md font-medium rounded-3xl ${errors.questionpoise && "border-l-[2rem] border-red-400"} transition-all`} type="number" placeholder='Poise and Personality' {...register("questionpoise", { required: true, max: 50, min: 0, maxLength: 2 })} />
